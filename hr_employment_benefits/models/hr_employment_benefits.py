...
class HrEmploymentBenefits(models.Model):
    ...
    # Aqui esta el conejo
    salaries_ids = fields.One2many(comodel_name='hr.employment.benefits.salaries', inverse_name='slip_id', string='Salarios')
        ...
        for rec in self:
            for line in rec.line_ids:
                debit = {
                    ...
                    'name': line.name,
                    'debit': line.total if line.total > 0.00 else 0.00,
                    'credit': line.total * -1 if line.total < 0.00 else 0.00,
                }
                lines.append((0,0,debit))
                credit = {
                    'account_id': line.salary_rule_id.account_credit.id,
                    'name': line.name,
                    'debit': line.total * -1 if line.total < 0.00 else 0.00,
                    'credit': line.total if line.total > 0.00 else 0.00,
                }
...
class HrEmploymentBenefitsLines(models.Model):
    ...
    config_ids = fields.Many2many('hr.employment.benefits.config', 'hr_lines_config_rel', 'line_id', 'config_id', string='Prestacion', copy=False)
    salary_rule_ids = fields.Many2many('hr.salary.rule', 'Salary Rules', compute="_compute_salary_rules")
    
    @api.onchange('company_id')
    def onchange_configs(self):
        if self.company_id:
            config_ids = self.env['hr.employment.benefits.config'].search([('company_id', '=', self.company_id.id)])
            self.config_ids = config_ids.ids or False
    ...
    def _compute_salary_rules(self):
        ...
        for rec in self:
            for conf in rec.config_ids:
                rules.append(conf.salary_rule_id.id)
            rec.update({
                'salary_rule_ids': rules or False,
            })

    @api.depends('slip_id.company_id')
    def _compute_company(self):
        company_id = False
        for rec in self:
            if rec.company_id:
                company_id = rec.company_id and rec.company_id.id
            else:
...
class HrEmploymentBenefitsSalaries(models.Model):
    ...
    slip_id = fields.Many2one(comodel_name='hr.employment.benefits', string='Pay Employment Benefits', required=True, ondelete='cascade')
    payslip_line_id = fields.Many2one('hr.payslip.line', string='Salario', required=True)
    ...
    currency_id = fields.Many2one("res.currency", string="Moneda", required=True,default=lambda self: self.env['res.currency'].search([('name', '=', 'GTQ')]).id)
    amount = fields.Monetary(string="Importe", related="payslip_line_id.amount")
    quantity = fields.Float(string="Cantidad", related="payslip_line_id.quantity")
    total = fields.Monetary(string="Total", related="payslip_line_id.total")
